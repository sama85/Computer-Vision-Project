...
def convert_to_float(string_to_convert):
    ...
    if ',' in string_to_convert:
        float_value = np.float32(string_to_convert.replace(',', '.'))
    else:
        float_value = np.float32(string_to_convert)
    return float_value
...
def update_rover(Rover, data):
    ...
    if Rover.start_time is None:
        ...
        Rover.samples_to_find = np.int32(data["sample_count"])
    ...
    # Near sample flag
    Rover.near_sample = np.int32(data["near_sample"])
    # Picking up flag
    Rover.picking_up = np.int32(data["picking_up"])
...
def create_output_images(Rover, Decider):
    ...
    cv2.putText(Rover.vision_image, "_ "+Decider.curr_state.__name__,
                (97, 20), cv2.FONT_HERSHEY_COMPLEX, 0.65, (55, 255, 17), 1)    
